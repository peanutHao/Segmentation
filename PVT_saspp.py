...
class saspp(nn.Module):
    ...
    def __init__(self, in_channels, branch_ratio=0.25):
        ...
        self.conv1 = nn.Conv2d(gc, gc, kernel_size=3, stride=1, padding=2, dilation=2)
        self.conv2 = nn.Conv2d(gc, gc, kernel_size=3, stride=1, padding=4, dilation=4)
        self.conv3 = nn.Conv2d(gc, gc, kernel_size=3, stride=1, padding=8, dilation=8)
        self.conv4 = nn.Conv2d(gc, gc, kernel_size=3, stride=1, padding=12, dilation=12)
    ...
    def forward(self, x):
        ...
        x = torch.cat(
            (self.conv1(x_1), self.conv2(x_2), self.conv3(x_3), self.conv4(x_4)),
            dim=1,
        )
        x = self.conv(x)
        return x
...
class PVT_saspp(nn.Module):
    def __init__(self, dim=32, dims=[64, 128, 320, 512]):
        super(PVT_saspp, self).__init__()

        self.backbone = pvt_v2_b2()  # [64, 128, 320, 512]
        path = '/data/qianhao/Polyp-PVT-main/pretrained_pth/pvt_v2_b2.pth'
        ...
        self.InDD_c4 = saspp(c4_in_channels)
        self.InDD_c3 = saspp(c3_in_channels)
        self.InDD_c2 = saspp(c2_in_channels)
        self.InDD_c1 = saspp(c1_in_channels)
        ...
        self.linear_fuse34 = ConvModule(in_channels=dim * 2, out_channels=dim, kernel_size=1,
                                        norm_cfg=dict(type='BN', requires_grad=True))
        self.linear_fuse2 = ConvModule(in_channels=dim * 2, out_channels=dim, kernel_size=1,
                                       norm_cfg=dict(type='BN', requires_grad=True))
        self.linear_fuse1 = ConvModule(in_channels=dim * 2, out_channels=dim, kernel_size=1,
                                       norm_cfg=dict(type='BN', requires_grad=True))
